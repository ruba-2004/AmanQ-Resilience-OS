...
export function OSDesktop({
  ...
  const handleReopenDevice = useCallback(async () => {
    ...
    try {
      ...
      // Controller may be unreachable; still dismiss overlay
    }
    try {
      sessionStorage.setItem(getServerDismissedKey(serverContext), "1");
    } catch {
      // ignore
    }
    ...
    const showNotification = () => {
      if (forThisServer.length > 0) {
        ...
        addDesktopNotification({
          ...
          message: `Server ${serverContext} is back online.`,
          actions: [{ label: "OK", onClick: () => {} }]
        });
      }
    };
    window.setTimeout(showNotification, 400);
  }, [serverContext, doRecoverFromDraft]);
